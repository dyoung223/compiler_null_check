...
public class MySolver implements Flow.Solver {
  ...
  /**
   * Sets the analysis. When visitCFG is called, it will perform this analysis on a given CFG.
   *
  ...
  /** Returns whether a list of quads contains an entry or exit */
  ...
  /** Processes a quad in the specified direction. Return value is true if changes were made */
  private boolean transfer(Quad quad, boolean direction) {
    Flow.DataflowObject original, modified;
    if (direction) original = analysis.getOut(quad);
    else original = analysis.getIn(quad);
    analysis.processQuad(quad);

    if (direction) modified = analysis.getOut(quad);
    else modified = analysis.getIn(quad);

    return !modified.equals(original);
  }

  /**
   * Runs the solver over a given control flow graph. Prior to calling this, an analysis must be
   * registered using registerAnalysis
   *
    ...
    while (changesMade) {
      ...
      if (analysis.isForward()) {
        while (iter.hasNext()) // Iterate forward
        {
          ...
          if (transfer(quad, true)) changesMade = true;
        ...
        while (iter.hasPrevious()) // Iterate backward
        {
          ...
          if (transfer(quad, false)) changesMade = true;
